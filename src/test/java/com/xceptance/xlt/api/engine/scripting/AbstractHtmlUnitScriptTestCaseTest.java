...
public class AbstractHtmlUnitScriptTestCaseTest
{
    ...
    {
        final Field pkgDataField = ReflectionUtils.getField(TestContext.class, "LOADED_PKG_DATA");
        pkgDataField.setAccessible(true);
        ((LRUHashMap<?, ?>) pkgDataField.get(null)).clear();

        final Field scriptDataField = ReflectionUtils.getField(TestContext.class, "LOADED_DATA");
        scriptDataField.setAccessible(true);
        ((LRUHashMap<?, ?>) scriptDataField.get(null)).clear();

        final Field globalDataField = ReflectionUtils.getField(TestContext.class, "_globalData");
    ...
    public void testResolve() throws Throwable
    {
        ...
        try (final MockedStatic<TestDataUtils> mock = Mockito.mockStatic(TestDataUtils.class))
        {
            mock.when(() -> TestDataUtils.getGlobalTestData()).thenReturn(globalData);
            mock.when(() -> TestDataUtils.getTestData(TestInstance.class)).thenReturn(tcData);
            mock.when(() -> TestDataUtils.getPackageTestData(any(Class.class), anyString(), anyString()))
    ...
    public void testResolvePackageData() throws Throwable
    {
        ...
        try (final MockedStatic<TestDataUtils> mock = Mockito.mockStatic(TestDataUtils.class))
        {
            mock.when(() -> TestDataUtils.getGlobalTestData()).thenReturn(globalData);
            mock.when(() -> TestDataUtils.getTestData(TestInstance.class)).thenReturn(tcData);
            mock.when(() -> TestDataUtils.getTestData(TestModule.class)).thenReturn(modData);
            mock.when(() -> TestDataUtils.getPackageTestData(any(Class.class), anyString(), eq(testPkgName))).thenReturn(tcPackageData);
            mock.when(() -> TestDataUtils.getPackageTestData(any(Class.class), anyString(), eq(modPkgName))).thenReturn(modPackageData);
            mock.when(() -> TestDataUtils.getPackageTestData(any(Class.class), anyString(), not(startsWith(testPkgName))))
                .thenReturn(Collections.<String, String>emptyMap());
