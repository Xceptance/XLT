...
public class HtmlPageUtilsTest
{
    ...
    {
        try (final MockedStatic<XltRandom> mock = Mockito.mockStatic(XltRandom.class))
        {
            // always return the first element of the list
            mock.when(() -> XltRandom.nextInt(3)).thenReturn(0);
            mock.when(() -> XltRandom.nextInt(2)).thenReturn(0);
            mock.when(() -> XltRandom.nextInt(1)).thenReturn(0);
    ...
    public void testSelectRandomly_HtmlFormStringBooleanBoolean() throws Exception
    {
        ...
        // finally, a list of 6 and we pick us number 5 by mocking random
        {
            ...
            try (MockedStatic<XltRandom> mock = Mockito.mockStatic(XltRandom.class))
            {
                mock.when(() -> XltRandom.nextInt(6)).thenReturn(4);  // 6 is size, 4 is position, starting at 0
