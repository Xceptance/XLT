...
public class MultipartFileUploadTest extends AbstractXLTTestCase
{
    ...
    /**
     * Executes a multipart/form-data POST request using the given HTTP client backend,
     * transmitting both a traditional text field and a binary file field, and asserts
     * that the server successfully receives and parses the complete multi-boundary payload.
     *
     * @param httpClientName the configured HTTP engine property name (e.g. apache4, okhttp3, jdk)
     * @throws Exception if socket handling or file I/O fails during transmission
     */
    @Test
