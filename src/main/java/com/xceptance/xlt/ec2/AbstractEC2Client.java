...
abstract public class AbstractEC2Client
{
    ...
    public AbstractEC2Client(final String accessKey, final String secretKey) throws Exception
    {
        ...
        try
        {
            ...
            proxyConfigBuilder.scheme(awsConfiguration.getProtocol().toString());
            proxyConfigBuilder.endpoint(new URIBuilder().setHost(awsConfiguration.getProxyHost()).setPort(awsConfiguration.getProxyPort())
                                                        .build());
    ...
    private Ec2Client clientForRegion(final String regionName)
    {
        ...
        if (client == null)
        {
            client = Ec2Client.builder().httpClient(ApacheHttpClient.builder().proxyConfiguration(proxyConfig).build())
