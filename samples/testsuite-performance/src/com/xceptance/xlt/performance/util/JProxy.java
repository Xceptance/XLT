	...
	public void run()
	{
		try
		{
			...
			if (debugLevel > 0)
			{
				...
				final String url = RegExUtils.getFirstMatch(new String(request), "GET ([^ ]+) HTTP", 1);
				
				final StringBuilder logEntry = new StringBuilder(256);
				logEntry.append(Long.toString(startTime)); logEntry.append('|');
				logEntry.append(pSocket.getInetAddress().getHostAddress());  logEntry.append('|');
				logEntry.append(pSocket.getLocalPort()); logEntry.append('|');
				logEntry.append(url); logEntry.append('|');
				logEntry.append(requestLength); logEntry.append('|');
				logEntry.append(responseLength); logEntry.append('|');
				logEntry.append(Long.toString(endTime - startTime) +"ms");
				
				debugOut.println(logEntry.toString());
				debugOut.flush();
			}
			if (debugLevel > 1)
			{
