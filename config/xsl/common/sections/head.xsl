    ...
    <xsl:template name="head">
        ...
        <style type="text/css">
            .chart-group .chart img, .chart-group .echart {
                width: <xsl:value-of select="configuration/reportGeneratorConfiguration/chartWidth" />px;
                height: <xsl:value-of select="configuration/reportGeneratorConfiguration/chartHeight" />px;
            }
            #transaction-summary .chart-group .overview .chart img, #transaction-summary .chart-group .overview .echart {
                height: <xsl:value-of select="configuration/reportGeneratorConfiguration/chartHeight * 1.5" />px;
            }
            #agents .chart-group .memory .chart img, #agents .chart-group .memory .echart {
                height: <xsl:value-of select="configuration/reportGeneratorConfiguration/chartHeight * 2.3" />px;
            }
        </style>
