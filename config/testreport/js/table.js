 ...
 * @version 0.986
 *
 * @history 0.986 2013-07-25 The sorting order to use when auto-sorting can be configured now.
 * @history 0.985 2009-06-10 Fixed sorting with negative numbers bug (Rene Schwietzke)
...
var Table = (function(){
	...
	var table = {
		...
		AutoSortOrderPrefix:"table-autosort-order:",
		...
		if (tables!=null) {
			for (var i=0,L=tables.length; i<L; i++) {
				...
				if ((val = classValue(t,table.AutoSortColumnPrefix)) || (hasClass(t,table.AutoSortClassName))) {
					var desc = classValue(t,table.AutoSortOrderPrefix) === 'desc';
					table.autosort(t,{'col':(val==null)?null:+val, 'desc':desc});
				}
			...
			if (type!=null) {
				...
				if (args.col!=null) {
					if (args.col==table.getActualCellIndex(c)) {
						args.sorttype=Sort[String(type)];
					}
				}
			}
		} );
