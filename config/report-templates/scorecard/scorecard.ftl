        ...
        <div class="content">
            ...
            <#else>
                <div class="paragraph verdict">
                    <#assign points = rootNode.@points>
                    <#assign totalPoints = rootNode.@totalPoints>
                    <#assign pointsPercentage = rootNode.@pointsPercentage>
                    <#assign ratingId = rootNode.rating>
                    ...
                    <#if ratingDefinition?has_content>
                        Based on the result, the test
                        <#assign passed = true>
                        <#if rootNode.@testFailed == "true">
                            <#assign passed = false>
                        </#if>
                        
                        <#assign ratingFailsTest = (ratingDefinition.@failsTest == "true")>

                        <#if passed>
                            succeeded with ${ratingDescription}.
                        <#else>
                            <#if !ratingFailsTest>
                                <#-- Rating itself doesn't fail test, check for specific rules/groups -->
                                <#assign failedRules = []>
                                <#list configuration.rules.rule as r>
                                    <#if r.@failsTest == "true" && r.@enabled == "true">
                                        <#-- Check against results -->
                                        <#assign ruleFailed = false>
                                        <#-- rootNode is outcome. Navigate groups/group/rule -->
                                        <#list rootNode.groups.group as g>
                                            <#list g.rule as gr>
                                                 <#if gr.@["ref-id"] == r.@id && gr.@testFailed == "true">
                                ...
                                <#list configuration.groups.group as g>
                                    <#if g.@failsTest == "true" && g.@enabled == "true">
                                        <#assign groupFailed = false>
                                        <#list rootNode.groups.group as resG>
                                            <#if resG.@["ref-id"] == g.@id && resG.@testFailed == "true">
